...
class JarchiveParser:
    def choose_random_episode(self):
        ...
        valid_episode_html_urls = [html for html in episode_html_urls if
                                   'j-archive.com/showgame' in html.get('href')]
        ...
        if 'j-archive.com/showgame' in episode_url:
            return {
                ...
                'url': episode_url,
                'taped': random_episode_html_url[
                    'title'] if 'title' in random_episode_html_url.attrs else None
            }
    ...
    def get_all_clue_text(self, all_rows):
        ...
        for row in all_rows[1:]:
            ...
            for single_row_data in all_row_data:
                ...
                clue_value_daily_double_html = single_row_data.find('td', {
    ...
    def get_ordered_dollar_values(self, all_rows):
        ...
        for row in all_rows[1:]:
            ...
            for single_row_data in all_row_data:
                ...
                if clue_value_html:
                    all_dollar_values.append(int(clue_value_html.text[
                                                 1:]))  # Remove the $ and convert to int
        return list(set(all_dollar_values))

    def get_jeopardy_round(self, soup, is_double_jeop):
        round_name_html = 'double_jeopardy_round' if is_double_jeop else 'jeopardy_round'
        round_name_db = 'double' if is_double_jeop else 'single'
        clue_table = soup.find('div', {'id': round_name_html}).find('table',
                                                                    {'class': 'round'})
        rows = clue_table.find_all('tr', recursive=False)
        category_info = self.get_category_info(first_row=rows[0],
                                               round_type=round_name_db)
    ...
    def get_final_round(self, soup):
        clue_table = soup.find('div', {'id': 'final_jeopardy_round'}).find('table', {
            'class': 'final_round'})
        ...
        answer_html_str_bw_parens = answer_html_str[
                                    answer_html_str.find("(") + 1:answer_html_str.rfind(
                                        ")")]
        final_answer = BeautifulSoup(answer_html_str_bw_parens, 'lxml').find(
            'html').find('em', {'class': '\\"correct_response\\"'}).text
        return [{
            'question_and_answer': {'clue': clue_text,
                                    'answer': final_answer,
                                    'is_daily_double': False},
            'money': None,
            'category_info': {'col_order_index': None,
                              'text': category_name,
                              'round_type': 'final'},
        }]
