...
def load_db_for_source_game():
    ...
    source_game_id = db.execute_and_commit(
        f"INSERT INTO {JeopartyDb.SCHEMA}.{JeopartyDb.SOURCE_GAME} (taped_date, jarchive_id) VALUES (%s, %s)",
    ...
    for clue_details in random_game_info["clues"]:
        ...
        category_query = f"SELECT id FROM {JeopartyDb.SCHEMA}.{JeopartyDb.CATEGORY} WHERE text = %s AND round_type = %s"
